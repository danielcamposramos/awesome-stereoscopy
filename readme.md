...
**Can field-sequential 3D be saved?** Yes, as content. A field-sequential recording converts to side-by-side with stock FFmpeg: [`separatefields`](https://ffmpeg.org/ffmpeg-filters.html#separatefields) splits each frame into its fields in the stream's own order, and [`stereo3d`](https://ffmpeg.org/ffmpeg-filters.html#stereo3d) with input `al` (left eye in the first field, or `ar` when it is the second) pairs them side by side. Each eye keeps half the lines, and fifty or sixty fields become twenty-five or thirty stereo pairs a second, but both views come from the same instant and nothing is invented. Encode with x264's `frame-packing=3` and the SEI that lets a 3D television switch itself is written too. The chain is [checked frame by frame](https://github.com/danielcamposramos/sony-bravia-linux/blob/main/tools/fieldseq-3d/verify-fieldseq-to-sbs.sh) on synthetic PAL and NTSC clips in both field orders.
